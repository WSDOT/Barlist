...
// Type82.cpp : Implementation of CType82
#include "Type82.h"
#include <tchar.h>
#include "BarData.h"
...
void CType82::BuildBend()
{
   ...
   hookDeduct1 = IsZero( GetW() ) ? 0.00 : CFabricationConstraints::GetHookDeduction( barData, use, HookType::ht180 );
   hookDeduct2 = IsZero( GetX() ) ? 0.00 : CFabricationConstraints::GetHookDeduction( barData, use, HookType::ht180 );
   ...
   {
      SetStatusLevel( StatusType::stError );
      AddStatusMsg(_T("ERROR : %1 must be greater than %2"), _T("W"), bendDeduct1+hookDeduct1);
   }

   if ( x < 0 )
   {
      SetStatusLevel( StatusType::stError );
      AddStatusMsg(_T("ERROR : %1 must be greater than %2"), _T("X"), bendDeduct2+hookDeduct2);
   }
