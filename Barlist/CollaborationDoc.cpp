...
BOOL CCollaborationDoc::OpenTheDocument(LPCTSTR lpszPathName)
{
   ...
   else
   {
      ...
      CString strDelim((TCHAR)1); // Barlist 4 uses ascii char 1 as a delimiter
...
#endif
            // path::string()/wstring() picks the narrow/wide accessor to
            // match std::_tstring's actual (TCHAR-dependent) character type.
#if defined(UNICODE) || defined(_UNICODE)
            m_vFiles.push_back(full_path_name.wstring());
#else
      ...
      m_Barlist->SetProject(std::_tstring((LPCTSTR)vProperties[0]));
      m_Barlist->SetJobNumber(std::_tstring((LPCTSTR)vProperties[1]));
      m_Barlist->SetEngineer(std::_tstring((LPCTSTR)vProperties[2]));
      m_Barlist->SetCompany(std::_tstring((LPCTSTR)vProperties[3]));
      m_Barlist->SetComments(std::_tstring((LPCTSTR)vProperties[4]));
   }
...
BOOL CCollaborationDoc::SaveTheDocument(LPCTSTR lpszPathName)
{
   ...
   else
   {
      ...
      CString strProject(m_Barlist->GetProject().c_str());
      CString strJobNumber(m_Barlist->GetJobNumber().c_str());
      CString strEngineer(m_Barlist->GetEngineer().c_str());
      CString strCompany(m_Barlist->GetCompany().c_str());
      CString strComments(m_Barlist->GetComments().c_str());
      CString strProperties;
      strProperties.Format(_T("Properties%s%c%s%c%s%c%s%c%s%c"), strProject, (TCHAR)1, strJobNumber, (TCHAR)1, strEngineer, (TCHAR)1, strCompany, (TCHAR)1, strComments, (TCHAR)1);
...
BOOL CCollaborationDoc::LoadFiles()
{
   ...
   {
      CBarlist barlist;
      if (!ReadBarlistFromFile(strFile.c_str(), barlist))
      {
         return FALSE;
      }
      else
...
void CCollaborationDoc::Merge(CBarlist& newBarlist)
{
   CGroupCollection& groups = m_Barlist->GetGroups();
   CGroupCollection& new_groups = newBarlist.GetGroups();

   std::size_t nGroups = new_groups.Count();
   for (std::size_t grpIdx = 0; grpIdx < nGroups; grpIdx++)
   {
      auto new_group = new_groups.Item(grpIdx);
      ATLASSERT(new_group != nullptr);

      const std::_tstring& groupName = new_group->GetName();

      auto group = groups.Find(groupName);
      if (!group)
      {
         // group does not currently exist... create it
         group = groups.Add(groupName);
      }
      else
      {
         m_vDuplicateGroups.push_back(CString(groupName.c_str()));
      }

      CBarRecordCollection& barRecords = group->GetBarRecords();
      CBarRecordCollection& newBarRecords = new_group->GetBarRecords();

      std::size_t nBars = newBarRecords.Count();
      for (std::size_t barIdx = 0; barIdx < nBars; barIdx++)
      {
         auto newBar = newBarRecords.Item(barIdx);
         ATLASSERT(newBar != nullptr);

         auto clone = CopyBar(*newBar);
